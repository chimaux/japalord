...
const OtherUpdateTab = ({ props: { data } }) => {
  ...
  const renderItem = ({ item }) => {
    return (
      <TouchableOpacity
        className="flex-row bg-[#424141] my-2 mx-4 rounded-xl"
      ...
      >
        ...
        <View className="w-[59%] h-[100px] px-2 py-2">
          ...
          <View className="flex-row justify-between flex-wrap ">
            <Text
              className="text-gray-400 mb-[3px]"
              style={{ fontWeight: "bold" }}
            >
              {Article(item.content)}
            </Text>
            <Text className="text-gray-400" style={{ fontWeight: "bold" }}>
              {formatCreatedAt(item._createdAt)}
            </Text>
          </View>
        </View>
      </TouchableOpacity>
  ...
  const renderLoader = () => {
    return (
      <View>
        <ActivityIndicator size="large" color="#6e002a6a" />
      </View>
    );
  };
  const loadMoreItem = () => {
